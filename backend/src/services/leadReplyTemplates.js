...
// sourced from the operator Master KB (src/knowledge/operatorMasterKbV2.js) — NOT invented:
//   ДС: от 15 000 сом, ~2 недели, действует 3 года
//   СС: от 35 000 сом, 1–1.5 месяца, действует 1 год
//   Отказное письмо: 5 000 сом
//   Первый ПИ включён; доп. ПИ: ДС +7 000, СС +9 000
...
const APPLICATION_URL = process.env.LEAD_APPLICATION_URL || 'https://dokumenty.pro/zayavka';
...
const SERVICE_BLURB = {
  declaration:    'Декларация соответствия (ДС): стоимость от 15 000 сом, срок изготовления около 2 недель, действует 3 года.',
  certificate:    'Сертификат соответствия (СС): стоимость от 35 000 сом, срок от 1 до 1.5 месяцев, действует 1 год.',
...
const PI_EXPLAINER =
  ...
  'если разные составы товара: доп. ПИ для ДС — +7 000 сом, для СС — +9 000 сом. Точное количество ' +
  'зависит от составов и требований лаборатории.';
...
function render(kind, ctx = {}) {
  ...
  switch (kind) {
    ...
    case 'application_link':
      return `Чтобы начать, заполните короткую заявку: ${APPLICATION_URL}\n\n` +
    ...
    case 'reminder': {
      ...
      return `Напоминаем про оформление документа — заявку можно заполнить здесь: ${APPLICATION_URL}.${tail}`;
    }
